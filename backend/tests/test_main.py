...
def test_street_name_does_not_exist():
    ...
    expected_error = "Unfortunately 'VIA CASALSERUGO' is not present in our database"
...
def test_numbers_of_stations_does_not_exist():
    ...
    assert response.status_code == 200
    expected_error ="Unfortunately 'NONEXISTENTSTREET' is not present in our database."
    assert response.json() == expected_error
...
def test_socket_types_does_not_exist():
    ...
    expected_error = {"detail": f"Unfortunately 'NONEXISTENTZONE' is not present in our database."}
    assert response.json() == expected_error
